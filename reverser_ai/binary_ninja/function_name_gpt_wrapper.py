...
class FunctionNameGPTWrapper:
    ...
    def read_config():
        ...
        config["use_mmap"] = Settings().get_bool("mistral_ai.use_mmap")
        config["n_threads"] = Settings().get_integer("mistral_ai.n_threads")
        config["n_gpu_layers"] = Settings().get_integer(
            "mistral_ai.n_gpu_layers")
        config["seed"] = Settings().get_integer("mistral_ai.seed")
        config["verbose"] = Settings().get_bool("mistral_ai.verbose")
